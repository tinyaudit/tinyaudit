"""Tests for the ACSIncome (Folktables) loader.

These tests pass even when the real Folktables data is unavailable; the loader
...
from tinyaudit.data.folktables import _split_indices


@pytest.fixture(scope="module")
def test_split() -> tuple[pd.DataFrame, pd.Series, pd.DataFrame]:
    return load_folktables(split="test", seed=0)


@pytest.fixture(scope="module")
def train_split() -> tuple[pd.DataFrame, pd.Series, pd.DataFrame]:
    return load_folktables(split="train", seed=0)
...
def test_sensitive_columns_not_in_features(
    ...
    # Also make sure the raw ACS column names are absent.
    assert "SEX" not in features.columns
    assert "RAC1P" not in features.columns
...
def test_determinism(
    ...
    features_b, label_b, sensitive_b = load_folktables(split="test", seed=0)
...
def test_seed_changes_split() -> None:
    f0, _, _ = load_folktables(split="test", seed=0)
    f1, _, _ = load_folktables(split="test", seed=1)
...
def test_invalid_split_raises() -> None:
    with pytest.raises(ValueError, match="split must be"):
        load_folktables(split="validation", seed=0)
